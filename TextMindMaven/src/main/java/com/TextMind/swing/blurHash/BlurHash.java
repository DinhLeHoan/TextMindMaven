 ...
 */
package com.TextMind.swing.blurHash;
...
public class BlurHash {
	...
	public static String encode(int[] pixels, int width, int height, int componentX, int componentY) {
		...
		for (int j = 0; j < componentY; j++) {
			for (int i = 0; i < componentX; i++) {
				...
				Util.applyBasisFunction(pixels, width, height, normalisation, i, j, factors, j * componentX + i);
			}
		}
	...
	public static BufferedImage decodeAndDraw(String blurHash, int width, int height, double punch,
			...
			for (int i = 0; i < width; i++) {
				...
				for (int y = 0; y < sizeY; y++) {
					for (int x = 0; x < sizeX; x++) {
						double basic = Math.cos(Math.PI * x * i / width) * Math.cos(Math.PI * y * j / height);
				...
				pixels[pos++] = 255 << 24 | (SRGB.fromLinear(r) & 255) << 16 | (SRGB.fromLinear(g) & 255) << 8
						| (SRGB.fromLinear(b) & 255);
			}
		}
